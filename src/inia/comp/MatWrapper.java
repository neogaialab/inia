...
public enum MatWrapper {
    ...
    private static HashMap<String, Material> CASHED_MAT_MAP = new HashMap<>();
    private static HashMap<String, MatWrapper> CASHED_WMAT_MAP = new HashMap<>();
    private static Boolean CASHED_IS_NEW = null;

    MatWrapper(int subid, String... altEllipsis) {
        this.altArray = altEllipsis;
        this.subid = subid;
    }

    public static MatWrapper fromBukkit(Material mat) {
        String matName = mat.toString();
        try {
            return MatWrapper.valueOf(matName);
        } catch (IllegalArgumentException e) {
            for(MatWrapper wMat : MatWrapper.values()) {
                for(String wMatName : wMat.altArray) {
                    if(wMatName.equalsIgnoreCase(matName)) {
                        return wMat;
                    }
                }
            }
    ...
    public static Boolean isNewVersion() {
        if(CASHED_IS_NEW != null) return CASHED_IS_NEW;

        Material test = Material.getMaterial("RED_WOOL");
        CASHED_IS_NEW = test != null;

        return CASHED_IS_NEW;
    }

    public static String getMatId(String name, byte subid) {
        return name.toUpperCase() + "," + subid;
    }
    
    public static MatWrapper requestMat(String name, byte subid) {
        String matId = MatWrapper.getMatId(name, subid);
        if(CASHED_WMAT_MAP.containsKey(matId)) return CASHED_WMAT_MAP.get(matId);
        
        for(MatWrapper wMat : MatWrapper.values()) {
            for(String wMatName : wMat.altArray) {
                byte wMatSubId = (byte) wMat.subid;

                if(name.equals(wMatName) && wMatSubId == subid) {
                    String wMatId = MatWrapper.getMatId(wMatName, subid);
                    CASHED_WMAT_MAP.put(wMatId, wMat);
                    return wMat;
                }
            }
        }
    ...
    public Boolean isMatEquals(ItemStack is) {
        if(is == null) return null;

        Material isType = is.getType();
        int isSubid = (int) is.getData().getData();

        if(MatWrapper.isNewVersion()) return is.getType() == this.getHandle();
        if(isType == this.getHandle()) return isSubid == this.subid;

        MatWrapper wMat = MatWrapper.fromBukkit(isType);
        if(this.isDamageable(wMat)) return this.getHandle() == isType;
        
        return false;
    }

    public Boolean isDamageable(MatWrapper type) {
        if(type == null) return null;
        
        String[] split = type.toString().split("_");
        int length = split.length;

        String itemName = split[length - 1];

        MatDamageable[] items = MatDamageable.values();

        for(MatDamageable i : items) {
            if(itemName.equalsIgnoreCase(i.name())) return true;
        }
    ...
    public ItemStack getIs() {
        Material mat = this.getHandle();

        if(this.isNewVersion()) return new ItemStack(mat);
        return new ItemStack(mat, 1, (byte) this.subid);
    }

    public Material findMatByAltArray() {
        String matName = this.name();
        Material mat = null;

        for (int i = 0; i < altArray.length; i++) {
            mat = Material.matchMaterial(altArray[i]);
            if(mat == null) continue; 
            
            CASHED_MAT_MAP.put(matName, Material.valueOf(altArray[i]));
            break;
        }
    ...
    public Material getHandle() {
        ...
        mat = CASHED_MAT_MAP.get(matName);
        if(mat != null) return mat;

        mat = this.findMatByAltArray();
        return mat;
    }
}